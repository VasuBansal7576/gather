...
- One logical progression owner composes the existing execution claims and waiting-work ledger; no independent scheduler may dispatch the same work. A single in-process runner picks `queued`/resumable intents, executes step by step, persists each step's receipt id before starting the next, and is idempotent on restart: a step with a persisted receipt is skipped, never re-executed. A crash after provider success but before receipt persistence is uncertain: reconcile the stable operation identity before any retry; not-found alone does not prove no external effect.
...
- Preserve confirmed/cancelled lifecycle state when replaying completed work; cancellation blocks any unsent step, including after awaits.
