...
- `caseset-005-v1.json` — ADR-005 per-business regression case set
  (version `005-v1`): owner-confirmed expectations answered from the
  business KnowledgePort. Before/after runs must share this version;
  added cases change the denominator and are never presented as
  improvement. Scores describe only this case set and claim no causal
  business improvement.
- `evidence/` — rendered ADR-005 UI evidence (rule confirmation, evidence
  review, empty/error states, scoped concession form, eval trend) at
  desktop (1440px) and mobile (390px) widths. Generated by
  `tests/knowledge-rules-ui.test.ts` from the actual components; all
  content is fictional fixtures.
